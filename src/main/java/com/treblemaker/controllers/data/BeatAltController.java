...
@Controller
public class BeatAltController {
    ...
    @Autowired
    public BeatAltController(IAnalyticsVerticalCustomDal analyticsVerticalCustomDal, IBeatLoopsDal beatLoopsDal, IHarmonicLoopsDal harmonicLoopsDal, ISoftSynthsDal softSynthsDal) {
    ...
    @RequestMapping(value = "/beatalt/inputcount", method = RequestMethod.GET)
    @ResponseBody
    ...
    @RequestMapping(value = "/beatalt/data", method = RequestMethod.GET)
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws InterruptedException, IOException, URISyntaxException {
        ...
        try {
            ...
            if (NNSingelton.INSTANCE.beatAltCsvString == null || NNSingelton.INSTANCE.beatAltCsvString.equalsIgnoreCase("") || resultSetExpiry == null || (new Date()).compareTo(resultSetExpiry) > 0) {
...
//            sw
            ...
            if (updateString) {
                NNSingelton.INSTANCE.beatAltCsvString = sw.toString();
            }

            outputStream.write(NNSingelton.INSTANCE.beatAltCsvString.getBytes());
    ...
    public float[] createPrdectionRow(int beat_loop_id,
        ...
        for (BeatLoop beatLoop : beatLoops) {
            if (beatLoop.getId().equals(beat_loop_id)
                    ||beatLoop.getId().equals(beat_loop_alt_id)) {
                output.add(1f);
            } else {
                output.add(0f);
            }
    ...
    public StringWriter appendCSVRow(StringWriter sw, ResultSet resultSet) throws SQLException {
        ...
        for (BeatLoop beatLoop : beatLoops) {
            if (beatLoop.getId().equals(resultSet.getInt("beat_loop_id"))
                    ||beatLoop.getId().equals(resultSet.getInt("beat_loop_alt_id"))) {
                sw.write("1");
            } else {
                sw.write("0");
            }
